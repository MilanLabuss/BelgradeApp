...
public class RAdapter extends RecyclerView.Adapter<RAdapter.MyViewHolder> {
    ...
    public RAdapter(Context ct, String nm[], String des[], int img[] ){
        names=nm;
        descriptions=des;
        images=img;
        context=ct;
...
//Setting the text to the textView from the names array
        ...
        holder.mylayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent intent = new Intent(context, ExtraInformationActivity.class);
                intent.putExtra("Name", names[holder.getAdapterPosition()]);
                intent.putExtra("Description", descriptions[holder.getAdapterPosition()]);
                intent.putExtra("myImg", images[holder.getAdapterPosition()]);
             //   Bundle bundle = ActivityOptions.makeCustomAnimation(context, R.anim.slide_in_right, R.anim.slide_out_right).toBundle();
